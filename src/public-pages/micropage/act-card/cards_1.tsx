...
import {YIYE_NAME} from "@/config/page_config";
import {formatTime} from "@/utils/utils";
import {sensorGetCoupon, sensorParticipate} from "@/service/sensorRegister";
...
export default class Cards extends React.Component<any> {
    ...
    async playStart(state, index) {
        ...
        console.log({isCanPlay: this.state.isCanPlay})
    ...
    participateAct(index) {
        return new Promise(async resolve => {
            ...
            ApiRequest.participate({postParams}).then((result) => {
                ...
                let sort = null;
                let choseItem = null;
                const {awardIdx, givenType, config, awardType, givenRecId} = result.awards[0];
                ApiRequest.commonActivityInfo({postParams: {activityId: result?.record.activityId}}).then(res => {
                    sensorParticipate({
                        name: '抽奖',
                        props: {pageTitle: "卡牌屋"},
                    ...
                    if (awardType === 2) {
                       sensorGetCoupon('卡牌屋',{...config,...res?.basicInfo})
                    }
                })
    ...
    closePrizeDlg() {
        ...
        const thisPage = PageList[PageList.length - 1]
        thisPage.animate('.dialog-container', [
            {opacity: 1},
            {opacity: 0},
        ], 1000, function () {
            this.setState({
    ...
    openPrizeDlg() {
        console.log({"open:": this.state.prizeDlgFlag})
        const PageList = Taro.getCurrentPages();
        const thisPage = PageList[PageList.length - 1]
        thisPage.animate('.dialog-container', [
            ...
            <View className="fit overflow-auto relative-position bg-grey2">
                <View className="window-width relative-position">
                    <Image
                        ...
                        mode="widthFix" className="vertical-bottom"/>
                            ...
                            <View className="row b-r-l-100 r-py-12  r-px-24"
...

                                {
                                    userInfo?.cstType > 0 && <View className="font-size-22 r-pl-12 r-ml-12"
                                                                   ...
                                                                   onClick={() => WXSDK.link(`${actConfig?.award?.linkValue}`, {
                                                                       iactivityId: this.state.activityId,
                                                                       refPageName: '卡牌屋'
                                                                   })}>我的奖品</View>
                                }
                ...
                {/*demo 遮罩层*/}
                {
                    this.state.flag && <View
                        className={`mask-container  fixed-full`}
                        style={{background: "rgba(0,0,0,.9)", zIndex: 10}}/>
                }
...
function Dialog(props) {
    ...
    return <View
        className={`dialog-container  fixed-full z-max overflow-hidden ${this.props.prizeDlgFlag ? '' : 'invisible'}`}
        style={{background: "rgba(0,0,0,.9)", opacity: 0}}>
...
function CardItem() {
    ...
    return <View
        className={`item_card_${i} width-160 height-240 flex-center flex warp-box  absolute ${playBtnState == 2 ? "ani-style" : ""}`}
        onClick={onClick}
        style={{
            left: `${v.offsetX}rpx`,
            top: `${v.offsetY}rpx`,
            zIndex: v.checked ? 20 : 1
        }}>
        <View
            className={`  overflow-hidden   ${v.checked ? "animations" : ((v.state == 2 && ![1, 2].includes(playBtnState)) ? "card-mask" : "")}`}
            style={{
                width: "148rpx",
                height: "228rpx",
            }}>
